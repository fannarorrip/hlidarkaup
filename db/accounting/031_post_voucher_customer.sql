-- Add customer_id to acc.post_voucher so account sales link the voucher to a customer.
-- (Posted vouchers are immutable, so customer_id must be set at insert time.)
set search_path = acc, public;

drop function if exists acc.post_voucher(text, date, text, text, text, text, jsonb, uuid);
...
begin
  ...
  insert into acc.vouchers(series_code, voucher_number, voucher_date, voucher_type,
                           ...
                           reverses_voucher_id, customer_id, created_by, posted_at, posted_by)
  values (p_series_code, v_number, p_voucher_date, p_voucher_type,
          p_description, p_external_reference, 'posted',
          p_reverses_voucher_id, p_customer_id, p_user_id, now(), p_user_id)
  ...
  values (p_user_id, 'post_voucher', 'voucher', v_voucher.id::text,
          jsonb_build_object('series', p_series_code, 'number', v_number, 'type', p_voucher_type,
                             'amount', v_total_debit, 'reference', p_external_reference, 'customer', p_customer_id));
  return v_voucher;
end $$;
