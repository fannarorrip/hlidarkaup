...
export async function extractReceiptLines(opts: { files: ExtractFile[] }): Promise<ParsedInvoice> {
  ...
  content.push({
    ...
    text:
      ...
      '- GJÖLD (skilagjald/úrvinnslugjald o.þ.h. — EKKI vsk): þegar reikningur sýnir verð bæði MEÐ og ÁN gjalda (t.d. Ölgerðin: "Innk.verð m/afsl án gj." vs "Innk.verð m/afsl & gj." og "Stk.verð m/afsl & gj."), notaðu ALLTAF upphæðirnar MEÐ afslætti OG gjöldum — gjöldin eru hluti innkaupsverðsins.\n' +
      '- PAKKNINGAR: ef reikningur sýnir bæði fjölda EININGA (kassa) og "stk pr einingu", þá er "qty" = HEILDARFJÖLDI STYKKJA (fjöldi eininga × stk pr einingu) og "unitPrice" = verð per STYKKI (m/afsl & gj.) — svo kostnaður reiknist á söluvöruna, ekki kassann.\n' +
