...
export async function POST(req: NextRequest) {
  ...
  if (String(password).length < 12) return NextResponse.json({ error: "Lykilorð verður að vera a.m.k. 12 stafir" }, { status: 400 });
...
// Update a staff member's role / active state — or email them a password/activation link.
// The email is sent via Resend with a Supabase recovery link we generate ourselves (not
// Supabase's rate-limited mailer). sendReset also creates + links a missing Supabase
// account, which activates staff rows seeded without one.
export async function PATCH(req: NextRequest) {
  const { email, role, is_active, sendReset } = await req.json();
  ...
  // Send a password / activation email (also creates + links the Supabase account if the
  // staff row was seeded without one — this is what "activates" a not-yet-active member).
  if (sendReset) {
    const r = await sendStaffPasswordEmail(email);
    if (!r.ok) return NextResponse.json({ error: r.error ?? "Tókst ekki að senda póst." }, { status: 400 });
