...
export async function GET(req: NextRequest) {
  ...
  // Vefflokkasía: slug úr shop.web_categories — yfirflokkur tekur alla undirflokka sína með.
  const flokkur = (sp.get("flokkur") ?? "").trim().slice(0, 80);
  ...
  const rows = await query<{
    ...
    product_group: string | null; web_cat_name: string | null; stock_quantity: string; is_stock_controlled: boolean; image_url: string | null; total: string;
  }>(`
    select p.product_number, p.name, p.description, p.price_gross, p.product_group, wc.name as web_cat_name,
           p.stock_quantity, p.is_stock_controlled, p.image_url,
           count(*) over() as total
    from shop.products p
    left join shop.web_categories wc on wc.slug = p.web_category
    where p.is_active and p.price_gross > 0
      ...
      and ($4 = '' or p.web_category = $4
           or p.web_category in (select c.slug from shop.web_categories c where c.parent_slug = $4))
    order by p.name limit $2 offset $3`, [search, limit, offset, flokkur]);
  ...
  const products = rows.map((r) => ({
    ...
    category: r.web_cat_name ?? (r.product_group ? (CAT_NAMES[r.product_group] ?? r.product_group) : ""),
